...
export default async function handler(req, res) {
  // CORS (útil para testes via browser/Hoppscotch)
  ...
  // Autorização do seu proxy (chave compartilhada com o GPT)
  ...
  try {
    const {
      database_id,       // obrigatório (usaremos /v1/databases/{id}/query)
      filter,            // opcional - Notion query filter (formato nativo)
      sorts,             // opcional - Notion query sorts (formato nativo)
      page_size = 25,    // opcional
      start_cursor,      // opcional
      all = false,       // se true, pagina até o fim
      select_properties  // opcional - lista de campos para incluir no flatten
    } = req.body || {};

    if (!database_id) {
      res.status(400).json({ error: "Provide database_id" });
      return;
    }
    ...
    do {
      ...
      if (!r.ok) {
        ...
        throw new Error(`Query failed ${r.status}: ${t}`);
      }
  ...
  for (const [key, val] of Object.entries(props)) {
    ...
    switch (val.type) {
      ...
      case "multi_select":
        ...
        break;
      case "status":
        out[key] = val.status ? val.status.name : null;
        break;
      case "checkbox":
        out[key] = !!val.checkbox;
        break;
      case "date":
        out[key] = val.date ? { start: val.date.start, end: val.date.end } : null;
        break;
      case "people":
      ...
      case "phone_number":
        ...
        break;
      case "formula":
        out[key] = extractFormula(val.formula);
        break;
      default:
        out[key] = null;
    }
